...
import { SlidersHorizontal, CheckCircle2, AlertTriangle, Key, ArrowRight, Clock, ShieldCheck, Zap } from 'lucide-react';
...
export const ControlOfficeWhatIfSimulator: React.FC<ControlOfficeWhatIfSimulatorProps> = ({
  ...
  const conflictingTrainName = hasVipConflict ? '#12951 Mumbai Rajdhani (VIP Tier 1)' : 'None (Clear Corridor Gap)';
  const isFeasible = !hasVipConflict;
  const hmacToken = `hmac-sha256-verified-sim-token-${Date.now().toString().slice(-6)}`;
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-3xl overflow-hidden shadow-xs flex flex-col h-full select-none font-sans">
      {/* Header */}
      <div className="bg-slate-50 border-b border-slate-200 px-5 py-3 flex flex-wrap items-center justify-between text-xs">
        <div className="flex items-center space-x-2 font-bold text-slate-900 tracking-wide">
          <SlidersHorizontal className="w-4 h-4 text-blue-600" />
          <span>WHAT-IF DISRUPTION SANDBOX: {blockCode} ({sectionCode})</span>
        </div>
        <span className="text-[11px] font-mono text-slate-500 font-bold">
          BASE: {baseStart.slice(0, 5)} — {baseEnd.slice(0, 5)} ({durationMins}m)
        </span>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
              <span className="text-[10px] uppercase font-bold text-slate-500">Base Timetable Slot</span>
              <div className="text-sm font-black font-mono text-slate-800">
                {baseStart.slice(0, 5)} — {baseEnd.slice(0, 5)} IST
              </div>
              <span className="text-[10px] text-slate-500 font-mono">15-min statutory headway met</span>
            </div>
            ...
            <div className={`p-3.5 rounded-2xl border space-y-1 ${
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10px] uppercase font-bold text-slate-600">Simulated Adjusted Slot</span>
              ...
              <span className="text-[10px] text-slate-500 font-mono">
                {isFeasible ? '✓ Safe Buffer Verified' : '🚨 Conflict with VIP Train'}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs font-mono text-slate-500 font-bold">
              ...
              <span className="text-slate-900 font-bold font-sans text-xs">
                {shiftMinutes > 0 ? `+${shiftMinutes} min shift` : `${shiftMinutes} min shift`}
              </span>
          ...
          <div className={`p-3 rounded-2xl border flex items-center justify-between text-xs ${
            ...
            <div className="flex items-center space-x-2">
              {isFeasible ? <ShieldCheck className="w-4 h-4 text-emerald-600" /> : <AlertTriangle className="w-4 h-4 text-rose-600" />}
              <div>
                <span className="font-bold block text-slate-900">
                  {isFeasible ? 'Corridor Conflict Status: FEASIBLE' : 'Hard Timetable Violation: INFEASIBLE'}
                </span>
                <span className="text-[11px] text-slate-600">
                  Impacted Service: <strong className="font-mono">{conflictingTrainName}</strong>
                </span>
              </div>
            </div>
            <span className={`text-[10px] font-bold font-mono px-2 py-0.5 rounded-full ${
              ...
              {isFeasible ? '0 MIN VIP DELAY' : '30 MIN DELAY'}
            </span>
          </div>
        </div>
        ...
        <div className="pt-2">
          {isCommitted ? (
            <div className="p-3 rounded-2xl bg-emerald-100 text-emerald-900 text-xs font-bold text-center flex items-center justify-center space-x-1.5 font-mono">
              <CheckCircle2 className="w-4 h-4 text-emerald-600" />
              <span>Shift Committed! Live Gantt Updated.</span>
            </div>
          ) : (
            <button
              onClick={handleCommit}
              disabled={!isFeasible}
              className={`w-full py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center space-x-2 transition-all shadow-xs ${
                isFeasible
            ...
            >
              ...
              <span>{isFeasible ? 'Commit Shift to Live Timetable' : 'Infeasible Shift: VIP Conflict'}</span>
            </button>
          )}
        </div>
      </div>
