import React from 'react';
import { Layers, Clock, ShieldCheck, CheckCircle2, AlertTriangle, TrendingUp } from 'lucide-react';

interface ControlOfficeKpisProps {
  activeBlocksCount?: number;
  shadowSavedHours?: number;
  vipDetentionMinutes?: number;
  criticalityScore?: number;
}

export const ControlOfficeKpis: React.FC<ControlOfficeKpisProps> = ({
  activeBlocksCount = 20,
  shadowSavedHours = 5.3,
  vipDetentionMinutes = 0,
  criticalityScore = 171.6,
}) => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 select-none font-sans">
      {/* KPI 1: Active Scheduled Repair Windows */}
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col justify-between hover:shadow-sm transition-shadow">
        <div className="flex items-center justify-between text-slate-500 mb-2">
          <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">
            Scheduled Repair Windows
          </span>
          <span className="p-1.5 rounded-lg bg-blue-50 text-blue-700">
            <Layers className="w-4 h-4" />
          </span>
        </div>
        <div className="flex items-baseline space-x-2 my-1">
          <span className="text-3xl font-black text-slate-900 font-mono tracking-tight">
            {activeBlocksCount}
          </span>
          <span className="text-xs font-bold text-slate-400 uppercase">
            Windows
          </span>
        </div>
        <div className="text-[11px] text-slate-500 flex items-center space-x-1 pt-1.5 border-t border-slate-100">
          <span className="text-emerald-700 font-bold">100% Verified</span>
          <span>• 24-Hour Timetable</span>
        </div>
      </div>

      {/* KPI 2: Downtime Saved */}
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col justify-between hover:shadow-sm transition-shadow">
        <div className="flex items-center justify-between text-slate-500 mb-2">
          <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">
            Track Downtime Saved
          </span>
          <span className="p-1.5 rounded-lg bg-emerald-50 text-emerald-700">
            <Clock className="w-4 h-4" />
          </span>
        </div>
        <div className="flex items-baseline space-x-2 my-1">
          <span className="text-3xl font-black text-emerald-700 font-mono tracking-tight">
            +{shadowSavedHours.toFixed(1)}
          </span>
          <span className="text-xs font-bold text-slate-400 uppercase">
            Hours
          </span>
        </div>
        <div className="text-[11px] text-emerald-800 font-semibold pt-1.5 border-t border-slate-100 flex items-center space-x-1">
          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-700" />
          <span>55% Downtime Cut (Bundled)</span>
        </div>
      </div>

      {/* KPI 3: VIP Headway Protection */}
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col justify-between hover:shadow-sm transition-shadow">
        <div className="flex items-center justify-between text-slate-500 mb-2">
          <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">
            Passenger Train Delays
          </span>
          <span className="p-1.5 rounded-lg bg-blue-50 text-blue-700">
            <ShieldCheck className="w-4 h-4" />
          </span>
        </div>
        <div className="flex items-baseline space-x-2 my-1">
          <span className="text-3xl font-black text-blue-700 font-mono tracking-tight">
            {vipDetentionMinutes}
          </span>
          <span className="text-xs font-bold text-slate-400 uppercase">
            Minutes
          </span>
        </div>
        <div className="text-[11px] text-slate-500 flex items-center space-x-1 pt-1.5 border-t border-slate-100">
          <span className="text-blue-700 font-bold">Vande Bharat &amp; Rajdhani</span>
          <span>• 0 Min Delay</span>
        </div>
      </div>

      {/* KPI 4: Urgent Safety Tasks Solved */}
      <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs flex flex-col justify-between hover:shadow-sm transition-shadow">
        <div className="flex items-center justify-between text-slate-500 mb-2">
          <span className="text-xs font-bold text-slate-700 uppercase tracking-wide">
            Urgent Safety Tasks Solved
          </span>
          <span className="p-1.5 rounded-lg bg-slate-100 text-slate-700">
            <TrendingUp className="w-4 h-4" />
          </span>
        </div>
        <div className="flex items-baseline space-x-2 my-1">
          <span className="text-3xl font-black text-slate-900 font-mono tracking-tight">
            {criticalityScore.toFixed(0)}
          </span>
          <span className="text-xs font-bold text-slate-400 uppercase">
            / 200 Index
          </span>
        </div>
        <div className="text-[11px] text-slate-700 font-semibold pt-1.5 border-t border-slate-100">
          High-Priority Flaws Resolved First
        </div>
      </div>
    </div>
  );
};
export default ControlOfficeKpis;
