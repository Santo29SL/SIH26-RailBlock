...
import { Sparkles, Layers, Clock, TrendingUp, AlertTriangle, CheckCircle2, HelpCircle } from 'lucide-react';
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="w-full py-2.5 px-4 rounded-2xl bg-white border border-slate-200 text-slate-700 hover:text-blue-600 flex items-center justify-between text-xs font-semibold shadow-xs transition-all cursor-pointer"
      >
        <div className="flex items-center space-x-2">
          <HelpCircle className="w-4 h-4 text-blue-600" />
          <span>New to RailBlock? View 2-Minute Visual Guide</span>
        </div>
        <span className="text-[11px] font-mono text-blue-600">Show Guide &darr;</span>
      </button>
    );
  }

  return (
    <div className="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-xs space-y-4 select-none">
      {/* Header */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-2xl bg-rose-50/50 border border-rose-200 space-y-2.5">
            ...
            <div className="space-y-1.5 text-xs font-mono pt-1">
              <div className="p-2 rounded-xl bg-white border border-rose-200 text-rose-900 flex justify-between">
                <span>1. Track Machine Repair (TMS)</span>
                <span className="font-bold">2.5 hrs</span>
              </div>
              <div className="p-2 rounded-xl bg-white border border-amber-200 text-amber-900 flex justify-between">
          ...
          <div className="p-4 rounded-2xl bg-emerald-50/50 border border-emerald-200 space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-emerald-800 flex items-center space-x-1.5">
                ...
                <span>The RailBlock AI Method (Joint Shadow Block)</span>
              </span>
            ...
            {/* Unified Bar */}
            <div className="p-3.5 rounded-xl bg-white border-2 border-emerald-300 space-y-2 text-xs">
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-medium text-slate-600">
                <span className="px-2 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200">Track (Primary 2.5h)</span>
                <span>+</span>
                <span className="px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-800 border border-emerald-200">Signal (Shadow 1.5h)</span>
                <span>+</span>
                <span className="px-2 py-0.5 rounded-md bg-blue-50 text-blue-800 border border-blue-200">Electric (Shadow 1.5h)</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'stages' && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
            <span className="text-[10px] text-blue-600 font-bold block uppercase">1. Ingestion</span>
            <p className="text-slate-600">Reads defect reports from TMS, SMMS &amp; TDMS databases.</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
            <span className="text-[10px] text-purple-600 font-bold block uppercase">2. AI Risk Score</span>
            <p className="text-slate-600">Scores defect urgency (0-100) so safety-critical repairs go first.</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
            <span className="text-[10px] text-amber-600 font-bold block uppercase">3. Gap Finder</span>
            <p className="text-slate-600">Finds free time slots in the timetable without train traffic.</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
            <span className="text-[10px] text-emerald-600 font-bold block uppercase">4. AI Optimizer</span>
            <p className="text-slate-600">Schedules joint blocks with 100% VIP zero-detention guarantee.</p>
          </div>
        </div>
      )}
      ...
      {activeTab === 'glossary' && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
            <span className="font-bold text-blue-600 block">USFD Flaw</span>
          ...
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
            <span className="font-bold text-amber-600 block">Private Number (PN)</span>
            <p className="text-slate-600">Station Master legal code ensuring the track is disconnected safely.</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
            <span className="font-bold text-emerald-600 block">TSR Recovery</span>
