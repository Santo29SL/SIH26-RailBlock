...
export const GisTrackMap: React.FC<GisTrackMapProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-3xl overflow-hidden shadow-xs flex flex-col h-full select-none">
      {/* Header */}
      <div className="bg-[#002b49] text-white px-5 py-3 flex flex-wrap items-center justify-between text-xs font-mono">
        <div className="flex items-center space-x-2 font-bold uppercase tracking-wider">
          ...
          <span className="text-white text-xs">GEOSPATIAL GIS TRACK &amp; TRACTION POWER (FP/SP) MAP</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-[10px] font-bold">
          ...
          <div className="flex items-center space-x-1.5">
            ...
            <span>25 kV OHE POWER ZONE</span>
          </div>
        </div>
      </div>
      ...
      <div className={`${heightClass} w-full relative z-0`}>
        ...
        >
          {/* CartoDB Positron High-Contrast Tile Layer */}
          <TileLayer
            attribution='&copy; <a href="https://carto.com/">CartoDB</a>'
            url="https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png"
          />

          {/* Railway Corridor Track Polyline (Dual UP & DOWN Tracks) */}
          <Polyline
            positions={trackPath}
            pathOptions={{
              color: '#0f172a',
              ...
              >
                <Popup>
                  <div className="p-1 font-sans text-xs space-y-1">
                    ...
                    <div className="text-[11px] text-slate-500">MAS-AJJ Quadruple Line Junction</div>
                    <div className="text-[10px] text-emerald-700 font-mono font-bold">Line Clear: Signals Normal</div>
                  </div>
                </Popup>
              </Marker>
            ))}
              ...
              <Popup>
                <div className="p-1.5 font-sans text-xs space-y-1.5 min-w-[180px]">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-slate-900">{item.request.request_code}</span>
                  ...
                  <button
                    ...
                    className="w-full mt-1 py-1 rounded bg-blue-600 hover:bg-blue-700 text-white font-bold text-[10px] flex items-center justify-center space-x-1 cursor-pointer"
                  >
          ...
          {/* Live Animated Trains traversing the corridor */}
          <AnimatedTrainLayer />
        </MapContainer>
      </div>
