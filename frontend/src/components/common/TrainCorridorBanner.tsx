...
export const TrainCorridorBanner: React.FC<TrainCorridorBannerProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs select-none space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-4">
        ...
        <div className="max-w-2xl space-y-1">
          ...
          <p className="text-xs text-slate-600 leading-relaxed font-sans">
            Automatic Block Planning AI System eliminates decentralized manual disconnections. Ingests data from TMS, SMMS, TDMS, and COA. Evaluates critical asset CI for risk, and bundles them into one Shadow window that clears the timetable.
          </p>
        </div>

        {/* Right: 4 Statistical Metric Counters matching screenshot */}
        <div className="flex items-center space-x-6 text-right font-mono">
          ...
          <div className="h-7 w-[1px] bg-slate-200"></div>
          <div>
            <div className="text-lg font-black text-emerald-600 leading-none">17</div>
            <div className="text-[10px] text-slate-400 font-bold mt-0.5">OPTIMIZATIONS</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-[11px] font-mono text-slate-500 pt-1 border-t border-slate-100">
        ...
        >
          <RotateCw className={`w-3.5 h-3.5 ${isScanning ? 'animate-spin text-blue-600' : ''}`} />
