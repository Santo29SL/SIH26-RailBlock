...
export const GanttPage: React.FC = () => {
  ...
  const handleInjectDelay = (trainNumber: string, delayMinutes: number) => {
    ...
    if (delayMinutes > 0) {
      setToastMessage(`🚨 Telemetry Alert: Train #${trainNumber} delayed by +${delayMinutes}m. Schedule timeline updated!`);
    } else {
  ...
  const handleAutoReschedule = (trainNumber: string) => {
    ...
    setToastMessage(`✓ 1-Click Automated Rescheduler Applied: Repair window rescheduled with 0 VIP delay!`);
    setTimeout(() => setToastMessage(null), 5000);
  };
    ...
    <div className="space-y-6 select-none font-sans">
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 sm:p-6 shadow-xs flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900">24-Hour Corridor Schedule &amp; Timetable</h2>
            <p className="text-xs text-slate-500">
              Live timetable comparing scheduled passenger &amp; freight trains against synchronized multi-department track repair windows
            </p>
          </div>
        </div>
