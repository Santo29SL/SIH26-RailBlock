    ...
    <div className="space-y-6 select-none font-sans">
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 sm:p-6 shadow-xs flex flex-wrap items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="text-base font-bold text-slate-900">
                Safety Approvals &amp; Clearances (Form T/351 &amp; T/D 602)
              </h2>
            ...
            <p className="text-xs text-slate-500">
              Department engineer approvals, Station Master safety codes (Private Numbers), and track access signoffs
            </p>
          </div>
        </div>
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 font-bold text-xs flex items-center space-x-1.5 transition-all cursor-pointer shadow-xs"
        >
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl overflow-hidden shadow-xs">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
            Track Repair Windows &amp; Station Master Safety Codes
          </h3>
          <span className="text-[10px] text-slate-500 font-mono font-bold">
            {displayBlocks.length} Repair Windows Ready For Signoff
          </span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-100/80 text-slate-700 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200">
              <tr>
                <th className="p-3.5">Window Code</th>
                <th className="p-3.5">Corridor Route</th>
                <th className="p-3.5">Track Affected</th>
                <th className="p-3.5">Timetable Slot</th>
              ...
              {displayBlocks.map((b) => (
                <tr key={b.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-3.5 font-mono text-slate-900">
                    {b.start_time?.slice(0, 5)} — {b.end_time?.slice(0, 5)} ({b.duration_minutes}m)
                  </td>
                  <td className="p-3.5">
                  ...
                  <td className="p-3.5 text-right">
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs shadow-xs transition-colors cursor-pointer"
                    >
                      Open Safety Notice (Form T/351)
                    </button>
                  </td>
                </tr>
              ))}
