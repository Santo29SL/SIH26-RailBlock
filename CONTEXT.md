...
**Forecast Freight Movement (Goods Trains Forecast)**:
Anticipated goods-train rake paths generated by the Control Office (flagged `FORECAST_FREIGHT`) that occupy future corridor capacity in monthly views before publication in timetables.
_Avoid_: Phantom train, speculative run, fake freight
...
**Availability-Impact Dimension**:
The speed-restriction delta ($\text{MPS} - v_{\text{TSR}}$) that measures the direct reduction in corridor throughput capacity caused by degraded assets, prioritized independently of immediate failure probability.
_Avoid_: Speed penalty, slowdown score, capacity drag
...
### Optimization, Planning & AI Governance
...
**Hazard Function with Domain Randomization**:
The synthetic labeling mechanism modeling non-linear asset degradation physics and feature interactions across randomized coefficient priors and latent railway section regimes.
_Avoid_: Ground truth formula, synthetic regression label

**Isotonic Calibration**:
Non-parametric monotonic post-processing that maps raw boosting outputs into well-calibrated true 30-day failure probabilities.
_Avoid_: Probability scaling, sigmoid fitting

**Monotone Positive Constraint**:
An algorithmic inductive bias enforced on tree boosting models guaranteeing that worsening asset degradation strictly produces non-decreasing risk scores.
_Avoid_: Monotonic heuristic, feature clamp
